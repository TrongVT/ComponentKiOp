...
import ColumnChartStackItem from './column-chart-stack-item'
  ...
  renderColumns (fadeAnim) {
    ...
    if (seriesCount > 0) {
      ...
      for (let i = 0; i < dataCount; i++) {
        renderColumns.push(
          <ColumnChartStackItem key={i} seriesArray={this.state.sortedData}
            dataIndex={i}
      ...
      for (let i = 0; i < this.state.sortedData.length; i++) {
        ...
        tooltipRenders.push(
          <View key={'tooltipText-' + i} style={{flexDirection: 'row', paddingLeft: 5, alignItems: 'center'}}>
            <View style={{backgroundColor:"#000", marginRight:3,    borderRadius: 2}} >
            ...
            <View style={{backgroundColor:"#000",marginRight:3,marginLeft:3,    borderRadius: 2}} >
              <View style={[styles.tooltipColor, {backgroundColor: !series.seriesColor ? this.props.primaryColor : series.seriesColor, opacity:0.7}]} />
            </View>
            <Text style={styles.tooltipValue}>{numberWithCommas(series.data[selectedIndex]['y2'], false)}</Text>
          </View>
        )
      }
      return (
